...
export default function DesignEditor({ ai, setAi, prompt, setPrompt }: Props) {
  ...
  const wheel=(e:React.WheelEvent)=>{if(e.ctrlKey||e.metaKey){e.preventDefault();setZoom(z=>Math.max(MIN_ZOOM,Math.min(MAX_ZOOM,z*(e.deltaY > 0 ? 0.92 : 1.08))))}else setPan(p=>({x:p.x-e.deltaX,y:p.y-e.deltaY}))};
