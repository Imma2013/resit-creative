...
import { CalendarDays, Clapperboard, Image, MessageSquare, Settings, Sparkles, Send, Upload, Plus, ChevronDown, Search, Command, Bell } from "lucide-react";
...
export default function Home() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span>R</span></div>
      <div className="nav-section-label">Workspace</div>
      {nav.map(({ id, label, icon: Icon }) => <button key={id} className={`nav-item ${tab === id ? "active" : ""}`} onClick={() => setTab(id)}><Icon size={18}/><span>{label}</span></button>)}
      <div className="sidebar-spacer" />
      <button className={`nav-item ${tab === "settings" ? "active" : ""}`} onClick={() => setTab("settings")}><Settings size={18}/><span>Settings</span></button>
      <div className="avatar">L</div>
    </aside>
    <main className="main">
      <header className="topbar">
        <div className="top-left"><button className="workspace-switcher"><span className="workspace-mark">R</span><span>Resit Creative</span><ChevronDown size={14}/></button><div className="top-divider"/><span className="crumb">{tab === "design" ? "Untitled design" : tab[0].toUpperCase()+tab.slice(1)}</span></div>
        <div className="top-center"><button className="command-search"><Search size={14}/><span>Search</span><kbd><Command size={10}/> K</kbd></button></div>
        <div className="top-actions"><button className="icon-top" aria-label="Notifications"><Bell size={16}/></button><button className="ghost"><Upload size={14}/> Import</button><button className="primary"><Plus size={14}/> New</button></div>
      </header>
...
function DesignView({ ai, setAi, prompt, setPrompt }: { ai: boolean; setAi: (v: boolean) => void; prompt: string; setPrompt: (v: string) => void }) {
  return <div className="design-workspace"><DesignEditor ai={ai} setAi={setAi} prompt={prompt} setPrompt={setPrompt} /></div>;
}
